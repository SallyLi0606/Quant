...
class SimpleLSTM(nn.Module):
    ...
    def forward(self, x):
        device = x.device
        h0 = torch.zeros(self.num_layers, x.size(0), self.hidden_size).type(x.dtype).to(device)
        c0 = torch.zeros(self.num_layers, x.size(0), self.hidden_size).type(x.dtype).to(device)
